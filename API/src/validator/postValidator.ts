...
class PostValidator {
  checkCreate() {
    return [
      ...
      body("idUser")
        .isEmpty()
        .withMessage(
          "L'identifiant de l'utilisateur ne peut pas être ajouté par un tiers"
        ),
      body("plantOrigin")
        .isString()
        .withMessage(
          "L'origine de la plante doit être une chaîne de caractères"
        )
        .notEmpty()
      ...
      body("plantRequirements")
        .isString()
        .withMessage(
          "Les besoins de la plante doivent être une chaîne de caractères"
        )
        .notEmpty()
  ...
  checkIdParam() {
    return [param("id").isInt().withMessage("L'id doit être un nombre entier")];
  }
}
