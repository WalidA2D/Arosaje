...
const defaultPP =
  "https://firebasestorage.googleapis.com/v0/b/api-arosa-je.appspot.com/o/profilepictures%2Fdefault_pp.png?alt=media&token=2415598b-9c6f-45a1-a9ad-c555cec6c3c6";
...
class UserController {
  async create(req: Request, res: Response) {
    ...
    try {
      ...
      if (email != null) return res.status(417).json({ msg: "Email déjà existant", status: 417 });
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      if (!record) return res.status(404).json({ msg: "Cible non trouvée" });
  ...
  async delete(req: Request, res: Response) {
    try {
      ...
      if (!record) return res.status(500).json({ status: 500, msg: "Can not find existing record" });
