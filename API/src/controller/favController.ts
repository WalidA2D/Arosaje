...
class FavController {
  async add(req: Request, res: Response) {
    try {
      ...
      const { id } = req.body;
      if (!id) return res.status(404).json({ success: false, msg: "Aucun ID de Post fourni" });

      if (await FavInstance.findOne({ where: { idPost: id, idUser: user.dataValues.idUsers } }))
        return res.status(413).json({ success: false, msg: "Ce post est déjà parmi les favoris" });
  ...
  async delete(req: Request, res: Response) {
    try {
      ...
      if (favoriPointed.dataValues.idUser != user.dataValues.idUsers)
        return res.status(413).json({ success: false, msg: "Droit requis" });
