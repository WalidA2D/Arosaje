...
function ConvScreen() {
    return (<Stack.Navigator initialRouteName="Conversations" screenOptions={{
            headerStyle: {
...
export function ExploreScreen() {
    ...
    const fetchConversations = () => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const response = yield fetch(`${apiUrl}/conv/read/${userId}`, options);
            const data = yield response.json();
            if (data.success) {
                const apiUsers = data.conversations.map((conv) => ({
    ...
    const handleSelectUser = (id) => {
        if (id !== 0) {
            setSelectedUsers((prevSelectedUsers) => prevSelectedUsers.includes(id)
                ? prevSelectedUsers.filter((convId) => convId !== id)
                : [...prevSelectedUsers, id]);
        }
    };
    const handleDeleteSelectedUsers = () => __awaiter(this, void 0, void 0, function* () {
        Alert.alert('Supprimer les conversations', `Êtes-vous sûr de vouloir supprimer les conversations sélectionnées ?`, [
            {
                text: 'Annuler',
                style: 'cancel',
            },
            {
                ...
                onPress: () => __awaiter(this, void 0, void 0, function* () {
                    ...
                    for (const convId of selectedUsers) {
                        ...
                        try {
                            const response = yield fetch(`${apiUrl}/conv/delete/${convId}`, options);
                            const data = yield response.json();
                            if (data.success) {
    ...
    return (<View style={styles.container}>
      {showSearchBar && (<View style={styles.searchBar}>
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh}/>}>
        {filteredUsers.map((user) => (<Pressable key={user.id} onLongPress={() => handleLongPressUser(user.id)} onPress={() => isSelecting
            ...
            <View style={styles.chatItem}>
              ...
              {user.userName !== 'Chatbot' && (<Pressable onPress={() => navigation.navigate('Profil', { idUser: user.idUser })}>
                  <Icon name="user" size={20} color="#668F80" style={{ marginLeft: 10 }}/>
                </Pressable>)}
            </View>
          </Pressable>))}
      </ScrollView>
      {isSelecting && selectedUsers.length > 0 && (<View style={styles.selectionButtonsContainer}>
