...
export default function ChatbotScreen() {
    ...
    useFocusEffect(React.useCallback(() => {
        navigation.setOptions({
            headerRight: () => (<Pressable onPress={resetConversation} style={{ marginRight: 10 }}>
    ...
    const handleUserSelection = (userMessage) => {
    ...
    const handleLinkPress = (text) => {
        if (text.includes('tel:')) {
            Alert.alert('Contact Support', `Voulez-vous appeler 07 69 20 47 49 ou envoyer un email?`, [
                {
                    text: 'Appeler',
            ...
            Alert.alert('Contact Support', `Voulez-vous envoyer un email à ${emailAddress}?`, [
                {
                    text: 'Envoyer un email',
            ...
            Alert.alert('Consulter le site', `Voulez-vous consulter le site ?`, [
                {
                    text: 'Consulter',
    ...
    return (<KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.container}>
        <ScrollView style={styles.messagesContainer} ref={scrollViewRef} contentContainerStyle={{ flexGrow: 1, justifyContent: 'flex-end' }}>
          {messages.map(message => (<Pressable key={message.id} onPress={() => message.sender === 'bot' && handleLinkPress(message.text)} style={[
        ...
        <View style={styles.optionsWrapper}>
          <ScrollView horizontal style={styles.optionsContainer} contentContainerStyle={styles.optionsContentContainer} showsHorizontalScrollIndicator={false}>
            {chatbotPrompts.map(option => (<Pressable key={option} style={styles.optionButton} onPress={() => handleUserSelection(option)}>
