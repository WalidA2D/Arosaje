...
export default function PubEspece() {
    ...
    return (<View style={styles.container}>
      <FlatList ref={flatListRef} data={species} keyExtractor={(item, index) => item.id.toString() + index.toString()} ListFooterComponent={<View style={styles.footerPadding}/>} renderItem={({ item }) => (<View style={styles.item}>
            ...
            {item.id !== -1 && (<Icon onPress={() => showSpeciesDetails(item.id)} color='black' size={32} name="information-circle-outline" type="ionicon" iconStyle={{ paddingTop: 15 }} containerStyle={{ backgroundColor: '#f9f9f9' }}/>)}
              <CheckBox checked={checkedId === item.id} onPress={() => handleCheckboxPress(item.id)} title={item.common_name} checkedColor='#668F80' uncheckedColor='#668F80' size={32} checkedIcon="dot-circle-o" uncheckedIcon="circle-o" iconRight containerStyle={{ backgroundColor: '#f9f9f9' }}/>
            </View>
          </View>)}/>
