...
export default function CalendarScreen() {
    ...
    useEffect(() => {
        const fetchEvents = () => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const response = yield fetch(`${apiUrl}/post/missions`, {
                    method: 'GET',
                    headers: {
                ...
                const fetchedEvents = data.missions.map((event) => ({
                    name: `${event.title}`,
    ...
    return (<View style={styles.container}>
      {currentDate && (<Calendar key={currentDate} current={currentDate} style={styles.calendar} theme={{
            ...
            }} markingType={'period'} markedDates={selectedDates} onMonthChange={(month) => setCurrentDate(`${month.year}-${month.month.toString().padStart(2, '0')}-01`)}/>)}
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        ...
        <View style={styles.eventsList}>
          {sortedEvents.length === 0 ? (<Text style={styles.noEventsText}>
              {showPastEvents ? 'Aucune mission passée.' : "Vous n'avez pas accepté de missions"}
            </Text>) : (sortedEvents.map((event, index) => {
            const eventStatus = getEventStatus(event);
            if (!showPastEvents && eventStatus === "")
                return null;
            return (<View key={index}>
                  <Pressable style={[styles.eventItem, selectedEvent === event.name && styles.selectedEventItem]} onPress={() => onEventPress(event)}>
